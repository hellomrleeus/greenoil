...
- **后端服务**：Cloudflare Workers + D1 (Google Maps API 代理、认证、餐馆分页查询)
...
参见 [worker/README.md](worker/README.md)。执行 `npx wrangler d1 migrations apply greenoil-restaurants --remote` 后，再执行 `npx wrangler deploy` 部署后端服务。
